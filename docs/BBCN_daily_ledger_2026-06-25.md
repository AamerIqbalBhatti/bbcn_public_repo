...
### E81. C9 — PATHWAY-TIER SWITCH ADDED (harness-only) — 2026-06-25 12:19 UTC
Added `PATHWAY_MODE = 'cde'` (default) | `'static'` in harness, mirroring `KERNEL_METHOD`.
Default 'cde' is byte-identical: regression PASSES, all counts == baseline. Static mode runs over
the full stage-eligible pool by pure max-mismatch (CDE narrowing bypassed); it is a benchmark, not
baseline-checked. controller.py and run_cde.py untouched. Harness now offers the full 2x2:
pathway cde|static x kernel stabilize|ranked; canonical default = CDE + Stabilization.
Static benchmark numbers (this run): E41 — TCGA apop 43% prolif 100% joint 57% durable 0%;
METABRIC apop 47% prolif 100% joint 43% durable 0%; ISPY2 apop 20% prolif 100% joint 53% durable 3%.
(E40 and E42 are unchanged from baseline, since the pathway tier does not affect those experiments.)
