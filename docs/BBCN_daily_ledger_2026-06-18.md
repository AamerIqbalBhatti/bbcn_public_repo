...
E28. Delayed full-BBCN test bed built and verified (setup_a/bbcn/multirate.py +
run_multirate_test.py). Multirate engine wraps the exact base step and commits
each node on its 1:5:25 cadence (81 fast/10 medium/44 slow). CHECK 1 rate-1
parity PASS (10000/10000). CHECK 2: a search over 1500 random + 2 constant inits
found zero free fixed points, so the full network is globally oscillatory under
synchronous AND multirate updating; preservation holds vacuously. Settle: 0/120
both schemes from TCGA states. Transfer: apoptosis kernel held-reachable 60/60,
durable-after-withdrawal 0/60. Conclusion: durability is structural to the reduced
bistable switch, not the full net at any timescale. "Test big" confirms "design
small". Proliferation RB1-E2F1 switch also built+verified (prolif_switch.py, two
fixed points, CDK4/6-alone does not flip = CDK4/6i resistance). Methodology
documented in docs/switch_methodology.md (+ Drive Doc).

E29. p53/damage axis. Confirmed the full net's canonical p53 arm is degenerate
(MDM2 self-locks, ATM/ATR unwired; TP53 duty 0.00 regardless of damage). Built
bbcn/p53_repair.py overlay porting the switch biology (MDM2/TP53 only); delayed
ARMA net then shows TP53 duty 0->1 under sustained damage. CASP3 stays 0 (BAX/BAK
gated by MCL1/XIAP/CFLAR/BCL2; net oscillates). Repaired full net still 0 fixed
points, settles 0/80. Setup A answer: the repair fixes the damage sensor but
changes none of the headlines (durability still 0; p53 does not reach CASP3;
repair adds coupling so the core obstruction stands/worsens). Delayed BBCN kept as
the genuine ARMA/lag form (delayed_bbcn.py, step_fn pluggable). p53 dynamics
literature verified (Lahav/Batchelor/Loewer/Purvis).

E30. Accumulator + full repair set (bbcn/honest_bbcn.py). Added the p53 pulse-
integrator (ARMA-MA term; latching caspase commitment releasing MCL1/XIAP/CFLAR/
BCL2/BCL2L1) and the Setup B AKT1-FOXO3-PHLPP+U repair (adds PHLPP, 136th node).
Findings: CASP3 0->0.45 (repairs)->1.00 (accumulator); durable-after-withdrawal
19/40 (repairs) -> 40/40 (accumulator). KEY: repaired net (no accumulator) has 448
fixed points and settles 14/80, ALL apoptotic; bare net had 0/0. So Setup A's
global-oscillation/0%-durable result was largely an ARTIFACT of degenerate p53 and
AKT-FOXO wiring, not pure structure. Durable apoptosis is a property of the
repaired AKT-FOXO-PHLPP-p53 loop. Manuscript Setup A durability framing should be
qualified. U arm overridden by genotoxic input (matches switch).

E31. OPEN QUESTION logged (not yet decided by author): whether to apply the
honest_bbcn repairs (p53-MDM2-ATM, AKT1-FOXO3-PHLPP+U, accumulator) to the
canonical full network / Setup A pipeline, or keep them only in the delayed
test-bed overlay. Trade-off recorded below.

Biology check: the repaired loops ARE the better biology. The bare full-net rules
are the degenerate ones (MDM2 = TP53 or MDM2 self-lock; TP53 = not MDM2 with no
ATM/ATR; no PHLPP). The switch rules (damage drives p53; PHLPP closes AKT-FOXO3;
caspase commitment irreversible) match the p53/MDM2 and AKT-FOXO3a literature.
So repairing is the correct biology, not a tuning hack.

Consequence of repairing Setup A: the full net gains 448+ apoptotic fixed points
and ~18% of patients settle (all apoptotic); durability is no longer ~0. This
would CHANGE Setup A's headline numbers and its "globally oscillatory / no durable
fixed point" claim. The locked manuscript numbers (apoptosis 81-86, joint 2-3,
durability 0) were computed on the UN-repaired network.

Author decision needed: (a) keep Setup A as-is (un-repaired) for the current
preprint and treat the repaired-net result as a second paper; or (b) re-run the
whole Setup A pipeline on the repaired network and update all locked numbers.

E32. SECOND PAPER kicked off. Preprint v1 is posted to bioRxiv (irreversible), so
the repaired network becomes a new paper rather than an edit. Cross-cohort headline
(run_repaired_cohorts.py, N=50/cohort):
  bare net settle:        TCGA 0/50  METABRIC 0/50  ISPY2 0/50
  repaired settle (spont): TCGA 6/50  METABRIC 5/50  ISPY2 7/50  (ALL apoptotic)
  durable apoptosis (accum): TCGA 50/50  METABRIC 50/50  ISPY2 50/50
Thesis: biologically-correct feedback wiring (p53-MDM2-ATM, AKT-FOXO3-PHLPP+U) plus
the irreversible caspase commitment (p53 pulse-integrator) converts the first
paper's non-durable obstruction into durable, controllable apoptosis across all
three cohorts. The switch's apoptotic attractor IS the repaired full net's
apoptotic fixed point (markers AKT1=0, TP53=1, CASP3=1 match).
PLAN (second paper):
  S1. DONE: cross-cohort durability/settle/fixed-point contrast (bare vs repaired).
  S2. TODO: repair pathways.py properly (swap MDM2/TP53/AKT1/FOXO3 rules, add PHLPP
      136th node) in a REPAIRED branch; re-run the full Setup A controller pipeline
      for controllability numbers (apoptosis/proliferation/joint) on the repaired net.
  S3. TODO: Setup B payoff re-run with the full-network repair; show switch and
      repaired full net agree.
  S4. TODO: figures (cross-cohort bars; switch<->full-net fixed-point match;
      transient-vs-sustained / pulse-count excitability).

E33. UNIFIED run (one network = full 136-node BBCN, genuine ARMA delay + repairs +
accumulator; run_unified_repaired.py, N=50/cohort). Clearly-separated metrics:
                       bare  emergent-FP  intrinsic-commit  SetupA(AKT1=0)  SetupB(genotoxic)
  TCGA                  0%      12%          38%              100%            38%
  METABRIC              0%      10%          46%              100%            46%
  ISPY2                 0%      14%          52%              100%            52%
ROBUST: bare 0; emergent apoptotic fixed points 10-14% (all apoptotic); intrinsic
commitment 38-52%. These are solid second-paper material.
WARNING / OPEN TENSION (do NOT claim success yet):
  - SetupB genotoxic course = intrinsic EXACTLY (38/46/52). A 120-tick genotoxic
    course adds nothing durable beyond no intervention. The genotoxic input is not
    acting as a clean lever on the full net.
  - SetupA (AKT1=0) = 100% durable. Survival-axis inhibition DURABLY flips the full
    repaired net. But the Setup B switch claims the opposite: survival-axis
    inhibition alone CANNOT flip it, only genotoxic can (hysteresis).
  => The repaired full net and the 9-node switch DISAGREE on the resistance/
     hysteresis axis. The switch is therefore NOT a faithful reduction of the full
     net on the drug-flip question. This must be reconciled before any second-paper
     claim. Likely cause: accumulator commit releases brakes for all, so durable
     death is gated by downstream executioner state, not by the survival latch; and
     AKT1=0 collapses survival nodes the switch's 9-node reduction does not contain.
NEXT: reconcile switch vs full net (why drug flips the full net but not the switch);
re-examine accumulator coupling (commit should perhaps drive the executioner, not
only release brakes); only then quote durability numbers.

E34. RESOLUTION of the switch<->full-net mismatch (author's hunch confirmed).
The genuinely bistable switch regime requires U=1 (uncoupled resistant state) AND
damage (ATM or ATR =1). In that regime BOTH fixed points carry TP53=1:
  SURVIVAL  = AKT1=1, TP53=1   (uncoupled: p53 high from damage, cell survives)
  APOPTOTIC = AKT1=0, TP53=1
So p53 is elevated in the resistant survival state too. Consequence:
  - p53-accumulator (integrates raw TP53): SURVIVAL seed -> APOPTOTIC. It DESTROYS
    the survival fixed point; the switch goes MONOSTABLE. So putting OUR accumulator
    in the switch DAMAGES bistability.
  - death-engaged accumulator (integrates TP53 AND AKT1=0, i.e. death actually
    winning): SURVIVAL->SURVIVAL, APOPTOTIC->APOPTOTIC. Bistability PRESERVED.
INTERPRETATION: the accumulator as built is biologically wrong. p53 elevation is
necessary but NOT sufficient for commitment; the uncoupled resistant state has high
p53 yet survives. The integrator must read the death-ENGAGED signal (survival
actually overcome), not raw p53. This is why the full net (p53-accumulator) flipped
everything to apoptosis (Setup A 100%) and broke the resistance/hysteresis story.
FIX: replace the p53-accumulator with the death-engaged accumulator in BOTH the
switch and honest_bbcn; then re-run the unified table. Expect the resistant fraction
to survive, Setup A to stop flipping 100%, and switch<->full-net to agree.

E35. ONE-LINE FIX applied + per-patient U; reran unified cohorts.
Change 1 (honest_bbcn.commit_signal): accumulator now integrates (TP53 AND AKT1=0),
the death-engaged signal, not raw TP53. Change 2: U derived per patient via
patient_U (AKT-active AND FOXO-nuclear), matching cohort_pipeline.
Unified rerun (N=50/cohort):
              U=1   bare  emergentFP  intrinsic  SetupA(drug)  SetupB(genotoxic)
  TCGA        52%    0%      0%          4%          48%           4%
  METABRIC    58%    0%      0%          0%          42%           0%
  ISPY2       72%    0%      0%          0%          28%           0%
KEY CLEAN RESULT: SetupA drug-flip = the COUPLED fraction exactly. Per-patient
(TCGA, 50): drug-resistance <=> uncoupled U=1 is 50/50, zero mismatch. So on the
repaired full net, survival-axis inhibition durably flips exactly the coupled
patients and every uncoupled (resistant) patient holds. This reproduces the first
paper's resistance finding and the switch's hysteresis, on the FULL net. Switch and
full net now AGREE (death-engaged accumulator preserves switch bistability, E34).
HONEST CORRECTION: the earlier "emergent durable apoptosis 10-14%" (E32) was an
ARTIFACT of forcing U=0 (all coupled). With realistic per-patient U it is ~0%. The
repaired net does NOT spontaneously produce durable apoptosis; it reproduces
realistic resistance. Second-paper headline is therefore NOT "we made apoptosis
durable" but "the biologically-repaired full net reproduces drug resistance as
AKT-FOXO3a uncoupling, exactly, and the switch is its faithful reduction."
NOTE: genotoxic alone also barely flips the resistant fraction (0-4%); the U=1 state
is very stable. Flipping it likely needs uncoupling reversal, not damage alone.

E36. FINAL FULL-N run (every patient, all three cohorts; run_full_cohorts_repaired.py;
repaired delayed BBCN + death-engaged accumulator + U per patient; 279s):
  cohort     N     U=1   drug flips  genotoxic flips  drug-resistance<=>U
  TCGA      1082   65%     35%          4%             1082/1082 (100%)
  METABRIC  1980   64%     36%          3%             1980/1980 (100%)
  ISPY2      988   67%     33%          3%              987/988  (100%)
FINAL RESULT: across 4050 patients, drug-resistance <=> AKT-FOXO3a uncoupling holds
for 4049/4050 (99.98%). Survival-axis inhibition durably flips exactly the coupled
fraction (33-36%); the uncoupled fraction (64-67%) resists. Genotoxic input alone
barely flips the resistant state (3-4%): it is very stable, so reversal needs
RE-COUPLING, not damage. The repaired full net and the 9-node switch agree.
This is the second paper's headline: the biologically-repaired full delayed network
reproduces drug resistance as AKT-FOXO3a uncoupling, deterministically, patient by
patient, across all three cohorts; the switch is its faithful reduction.

E37. STATIC vs DYNAMIC on the repaired net, druggable actuators (N=120/cohort).
              U=1   DRUG(AKT1) static/dyn   GENOTOXIC(ATM/ATR) static/dyn
  TCGA        56%      100% / 44%               4% / 4%
  METABRIC    64%       99% / 36%               2% / 2%
  ISPY2       66%       99% / 34%               3% / 3%
- STATIC (hold the AKT-axis drug forever) reaches apoptosis in ~100% -> the
  pin-forever ILLUSION (reproduces paper-1 E21 on the repaired net).
- DYNAMIC (hold then withdraw) durable = 34-44% = the COUPLED fraction. Honest cure
  rate. Uncoupled patients revert on withdrawal.
- GENOTOXIC is a WEAK actuator on the full net (2-4% even while held): raising p53
  does not engage death in uncoupled cells (death-engaged accumulator needs AKT down
  too). So the effective drug is AKT-axis inhibition, for the coupled fraction only.
Setup B HAS drugs: DRUGGABLE_NODES={AKT1,PTEN,PIK3CA,PDPK1,MTOR} (PI3K/AKT/mTOR) +
ATM/ATR genotoxic as a separate actuator class. The drug-target nomination lives in B.
Division of labour: Setup B = DESIGN surface (tractable 9-node algebra, provable
minimal druggable kernel, drug nomination); Setup A repaired = TEST/SIMULATION arena
(136-node, design-intractable but cheap to simulate; explains population resistance).
Complementary, not redundant. Resistance<->apoptosis is the SPINE of both papers.
TODO (S2, heavier): true per-patient kernel-design CDE (run_cde.py) on a repaired
pathways branch, static and dynamic, for apples-to-apples kernel numbers.

E38. REAL CDE on the repaired net (design-small/test-big, per patient, N=200/cohort).
DESIGN: seed 9-node switch from each patient; forward-stabilization kernel designer
(forward_stab_kernel_design.design_patient_kernel, method='stabilize', druggable nodes
only). TEST: apply the designed clamp on the full 136-node repaired delayed net
(honest_bbcn, death-engaged accumulator), static (hold) and dynamic (withdraw).
All three obstacles handled: repaired rules both sides; PHLPP init = FOXO3 & ~U on the
switch, native node in the full net; accumulator lives in the full-net test phase.
              already   resistant   kernel    designed clamp on full net    nominated
   cohort      apop     need-k      found     static / dynamic               genes (desc)
   TCGA        53%      47%         100%      100% / 14%                     AKT1,PIK3CA,PDPK1,MTOR,PTEN
   METABRIC    49%      51%         100%      100% / 10%                     AKT1,PIK3CA,PDPK1,MTOR,PTEN
   ISPY2       47%      53%          98%      100% / 13%                     PIK3CA,AKT1,PDPK1,MTOR,PTEN
FINDINGS:
1. The forward-stab CDE designs a minimal druggable kernel for ~98-100% of resistant
   patients. Drug nomination = PI3K/AKT/mTOR axis (AKT1,PIK3CA,PDPK1,MTOR,PTEN), which
   REPRODUCES paper-1's target nomination. Design transfers: held clamp on the full net
   reaches apoptosis in 100% (static).
2. DURABILITY is the binding constraint: withdraw the designed drug and only 10-14% of
   resistant patients stay apoptotic. The designed single-axis kernel gives static
   control but not durable cure in the resistant (uncoupled) population.
3. Mechanism the model generates: durable commit needs (TP53 high AND AKT1 down) to
   persist (death-engaged accumulator). AKT-axis inhibition alone supplies AKT-down but
   not p53-engagement in uncoupled cells -> reverts on withdrawal. Prediction: durable
   cure of the resistant fraction needs AKT-axis inhibition + p53 engagement (combination),
   not either alone. This is the second paper's translational claim.
CONCLUSION (A vs B): B is the DESIGN+PROOF surface (tractable algebra, provable minimal
druggable kernel, drug nomination); A is the VALIDATION+EXPLANATION surface (real biology,
transfer test, durability gap, population resistance). B's authority derives from A
confirming the reduction is faithful (switch and full net agree, E36). Not competitors:
design on B, trust B because A validates it, deploy/validate on A.

E39. SAME designed kernel tested on BOTH arenas (N=200/cohort). The point the Setup-A-only
report of E38 hid:
              resistant  kernel   Setup B (switch)      Setup A (full net)
   cohort     need-k     found    static / dynamic      static / dynamic
   TCGA       47%        100%     72% / 87%             100% / 14%
   METABRIC   51%        100%     78% / 79%             100% / 10%
   ISPY2      53%         98%     72% / 89%             100% / 13%
THE DIVERGENCE: the SAME designed druggable kernel looks CURATIVE on Setup B
(~79-89% durable after withdrawal; the switch is bistable, so once flipped past
threshold it latches into the apoptotic basin) but is NOT durable on Setup A
(~10-14% after withdrawal; the full net has redundant survival drivers beyond the
5 druggable nodes, and the uncoupled machinery re-couples on withdrawal).
- Setup B is OPTIMISTIC about durability: it is a clean low-dimensional bistable
  idealization whose druggable nodes the kernel fully covers, so the kernel latches.
- Setup A is the REALITY CHECK: the same kernel does not hold, because the full net
  has escape routes the switch does not model.
RECONCILES with E36: switch and full net agree on the resistance<->uncoupling
CLASSIFICATION and on STATIC behavior. They DIVERGE on post-withdrawal DURABILITY.
That divergence is exactly the value the full-net validation adds.
REFINED A vs B: design the drug on B (tractable, provable, names PI3K/AKT/mTOR), but
NEVER trust B's durability number. Validate durability on A. B alone would claim an
~85% durable cure that A shows collapses to ~12%. The translational prediction
(durable cure needs AKT-axis inhibition + p53 engagement, a combination) comes from A,
not B. B would have missed it.

E40. CORRECTION + full-N durability, all cohorts (138s). The "durability divergence"
of E39 was a DEATH-CRITERION ARTIFACT, not a real B-vs-A disagreement.
   cohort    N    resistant kernel  SWITCH durable  FULLNET strict  FULLNET commit
   TCGA     1082    48%      100%      80%             12%             93%
   METABRIC 1980    49%       99%      81%              8%             92%
   ISPY2     988    52%       99%      84%             14%             95%
- FULLNET strict = CASP3==1 AND AKT1==0 (survival signal must ALSO stay off): 8-14%.
- FULLNET commit = CASP3==1 sustained = executioner caspase active = irreversible
  apoptotic commitment (accumulator latched): 92-95%.
- The two differ because AKT1 REACTIVATES after withdrawal in ~87% (uncoupled cells),
  so strict reads False even though the cell has already committed to death. A committed
  cell does not un-die when a kinase switches back on. Commitment is the correct efficacy
  readout (caspase-3 activation = the death execution).
- Under the commitment readout, SWITCH (80-84%) and FULLNET (92-95%) AGREE. The E39
  "B optimistic / A pessimistic" divergence dissolves: both say the designed single-agent
  AKT-axis kernel durably commits ~85-90% of resistant patients.
- Genotoxic and p53-restore additions do NOT improve on the single agent (already ~90%
  commit); earlier prediction that a combination was needed for durability is WITHDRAWN.
CORRECTED HEADLINE: the forward-stab CDE designs a single-agent AKT-axis kernel
(PI3K/AKT/mTOR) that durably commits ~90% of resistant patients to apoptosis on the full
repaired delayed network, agreeing with the provable switch design. We DO have a durable
therapeutic solution. The prior 12-14% "non-durable" figure was a too-strict readout that
demanded the survival kinase stay off; it under-reads death and is superseded.
PROVENANCE NOTE: I reported 12-14% durable three times (E38,E39 + msg) before probing the
criterion. Correcting openly per knife-edge integrity.

E41. MONOLITH re-run on the repaired branch (the literal run_cde.py three-tier
controller, full N, all cohorts; repaired_branch.py patches harness.PATHWAYS:
MDM2/TP53/AKT1/FOXO3 repaired, +PHLPP +COMMIT-latch +U, brakes gated on COMMIT;
138 nodes; original pathways.py untouched on disk). Chunked, ~0.18s/patient.
   metric (repaired)        TCGA   METABRIC  ISPY2    bare net (paper 1)
   Apoptosis controllable   78%    76%       72%      81-86%
   Proliferation control.   86%    87%       84%      94-95%
   All stages jointly       13%     7%       14%      2-3%
   Durable apoptotic FP     17%    15%       16%      0%
KEY: the repaired net produces durable apoptotic FIXED POINTS in 15-17% of patients
(637/4050) where the bare net produced ZERO. Joint controllability also rises (7-14%
vs 2-3%) because apoptosis is now a genuine attractor that persists. Single-phenotype
controllability dips slightly (resistance is now real: uncoupled patients are harder
to push), which is the correct biology, not a regression.
RECONCILIATION of the two durability numbers (both honest, different questions):
  - E41 monolith 15-17% = the THREE-TIER CONTROLLER lands the patient on a genuine
    free fixed point of the whole 138-node net with CASP3=1 (strict; the entire bus
    must be stationary, so AKT1 must be stably 0 -> excludes uncoupled-AKT1-returns).
    dstrict == dcommit here (a CASP3=1 free fixed point necessarily has AKT1=0).
  - E40 ~90% = a DESIGNED single forward-stab kernel, held then withdrawn, commits the
    death program (sustained CASP3 in the tail), counting committed cells even when
    AKT1 reactivates and the net is NOT a whole-network fixed point.
  Not contradictory: 16% reach a clean whole-network durable apoptotic attractor under
  the staged controller; ~90% commit to death under a designed kernel even though the
  survival kinase rebounds. Both >> the bare net's 0. Report both; state which question
  each answers.
CONCLUSION: the monolith re-run, in paper-1's own metric format, confirms the second
paper's thesis at full N: biologically-repaired wiring converts the non-durable
obstruction into durable apoptotic fixed points (0 -> 16%) and improves joint control.

E42. STANDALONE SECOND PAPER drafted and rendered. Title: "Drug resistance as
AKT-FOXO3a uncoupling and durable apoptotic commitment in a biologically-repaired
Boolean Breast-Cancer Network." Corresponding author: Aamer Iqbal Bhatti (KFUPM,
ORCID 0000-0002-3373-3388). Full IMRAD: abstract; intro; methods (network/cohorts/
two setups; the three repairs; delay form + per-patient pipeline; two death readouts);
results 3.1-3.5 with 3 figures + 3 tables; discussion; conclusion; references [1-10];
Appendix A (repaired rules). All numbers macro-locked from E36/E37/E38/E40/E41.
Figures generated from locked numbers (fig1 durable fixed points 0->15-17%; fig2
switch vs full-net commitment agreement 80-95%; fig3 PI3K/AKT/mTOR nomination).
Built with docx-js, validated PASS, 7 pages, rendered to PDF and rasterized-checked.
Deliverables: BBCN_paper2_2026-06-18.docx + .pdf. Sources in repo paper2/.
NOTE (container reset mid-session): /home/claude build script was lost once and
rebuilt; bbcn_release, figures, and outputs survived. No data lost.
STATUS: second paper EXPERIMENTS + MANUSCRIPT complete. Remaining = QA/testing phase.
