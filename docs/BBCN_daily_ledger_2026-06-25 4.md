...
### E80. C5 DROPPED; C9 SIMPLIFIED TO HARNESS-ONLY; AUTO-LEDGER CONVENTION

Two simplifications after auditing harness directly.

Harness is THREE-tier (outer CDE/stage-session loop, middle pathway resolution by mismatch, inner
kernel search), and it is self-contained for both switchable tiers:
- Kernel tier already switches in harness: `KERNEL_METHOD = 'stabilize'` (default) | `'ranked'`
  (Greedy Search). No work needed.
- Pathway tier: harness has the ingredients (`EXPECTED_PATHWAYS`, `_pathway_mismatch`,
  `_resolve_active`) but `_resolve_active` is always restricted to the CDE-chosen set. Static (pure
  max-mismatch over all stage-eligible pathways) is one flag away.

DECISION 1 — C5 DROPPED. Once the canonical engine is harness-only, `controller._kernel_for` and
`run_cde._find_kernel` are non-canonical paths. Harness already has its own complete kernel switch,
so merging the three kernel copies buys only tidiness across files we are sidelining. The kernel-code
duplication across harness / controller / run_cde is ACCEPTED (a smell, not a bug; the two non-harness
copies are reference-only). C5 is removed from the plan.

DECISION 2 — C9 IS HARNESS-ONLY. Instead of unifying two drivers, add `PATHWAY_MODE = 'cde'`
(default) | `'static'` inside harness, mirroring `KERNEL_METHOD`. Static makes `_resolve_active`
run over the full stage-eligible set by pure max-mismatch, bypassing the CDE narrowing. Default
`'cde'` keeps the canonical numbers frozen. Controller and run_cde are NOT touched. Harness alone
then offers the full 2x2: pathway `cde|static` x kernel `stabilize|ranked`, default = CDE +
Stabilization.

CONVENTION — AUTOMATED LEDGER UPDATE. From here, every Claude Code task self-documents: after its
regression passes, it appends its own dated entry to this ledger on the same branch, so code and
ledger land together in one merge. The human no longer hand-writes each close. CAVEAT: because
multiple branches append to the END of this same file, two branches landing close together can
produce a trivial conflict at the ledger tail (both want the last line). Resolve by keeping BOTH
entries in ascending E-number order; it is never a code conflict, only document-tail ordering.

### E81. C9 — PATHWAY-TIER SWITCH ADDED (harness-only) — done in Claude Code

Added `PATHWAY_MODE = 'cde'` (default) | `'static'` in harness, mirroring `KERNEL_METHOD`. Default
'cde' is byte-identical: regression PASSES, all counts == baseline. Static mode resolves over the
full stage-eligible pool by pure max-mismatch (CDE narrowing bypassed); it is a benchmark, not
baseline-checked. `controller.py` and `run_cde.py` untouched. Harness now offers the full 2x2:
pathway cde|static x kernel stabilize|ranked (Greedy Search); canonical default = CDE + Stabilization.

Static benchmark numbers, E41 (apop / prolif / joint / durable):
TCGA 43 / 100 / 57 / 0 ; METABRIC 47 / 100 / 43 / 0 ; ISPY2 20 / 100 / 53 / 3.

### E82. ANALYSIS — CDE vs Static (E41), and the `joint` metric clarified

CDE (frozen default) vs Static benchmark, E41 monolith:
- TCGA:     apop 70->43 | prolif 100 | joint 30->57 | durable 17->0
- METABRIC: apop 63->47 | prolif 100 | joint  3->43 | durable 13->0
- ISPY2:    apop 80->20 | prolif 100 | joint 20->53 | durable 10->3

Headline: durability is what the adaptive CDE buys. Under Static (pure max-mismatch) durable
collapses to 0/0/3% vs 17/13/10% under CDE; apoptosis also falls (sharpest ISPY2 80->20);
proliferation is 100% in both and does not discriminate. Real measured justification for
CDE + Stabilization being canonical, and Static earns its keep as the benchmark.

The `joint` metric, pinned from `PHENO_SCHEMA_V2['Terminal']`:
`joint = (evaluate_phenotype(final_bus, 'Terminal', scheme='v2') == 'PASS')`, which requires
`CASP3=1` (apoptosis) AND `CCND1=0,E2F1=0` (prolif-off) AND `STAT3=0,RELA=0` (resist-off). So joint
is the genuine all-three terminal verdict, read on the FINAL bus.

Why joint can exceed apop (not a paradox): `apop` is the Apoptosis_ON STAGE pass (v1, mid-sequence)
and that stage additionally fails on a proliferation contradiction (forbids CCND1/MYC/E2F1 on at
that moment); `joint` is read at the END after later stages drove proliferation off. Different
stage, scheme, and time point -> not nested. Static's joint(57) > apop(43) is real, not a bug.

Caveat for the paper: `joint` is a SNAPSHOT at the end of the control window; `durable` is whether
it survives the drug clearing (Static holds the joint state in 0% despite reaching it in 57%). So
joint alone is NOT a durability proxy -- `durable` is. Define joint as the final Terminal (v2)
verdict; do not equate it with apop, and do not read joint-above-apop as a contradiction. Verdict:
the column is trustworthy as defined; the interpretation was the only risk, now pinned.
...
**C5 — Merge the kernel-design copies. DROPPED (E80).** Superseded by the harness-only direction:
harness already has its own kernel switch, so the three-way kernel duplication across harness /
controller / run_cde is accepted (controller and run_cde are non-canonical). No merge.
...
**C9 — Pathway-tier switch, harness-only. DONE (E81).** `PATHWAY_MODE = 'cde'` (default) |
`'static'` added in harness; default byte-identical (regression PASS); static is a labelled
benchmark; controller.py and run_cde.py untouched. Harness now offers the full 2x2.
