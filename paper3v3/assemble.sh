#!/usr/bin/env bash
# Assemble the self-contained paper3v3 folder from the PIPELINE SOURCES.
# paper3v3 is a SIBLING of paper3: both draw from preprint/ and results/, so
# paper3v3 does not depend on paper3 and never lags behind it.
# Run from inside the repo (needs preprint/, results/, setup_a/ present).
set -e; cd "$(dirname "$0")"
mkdir -p figures

echo "[1/3] regenerate paper3v3-only artifacts (from ../setup_a/data/cde_vs_switch_summary.csv)"
...
# NOTE: appendix_repairs.tex is v3-specific, hand-maintained (transcribed from
# setup_a/bbcn/honest_bbcn.py). It is NOT copied from preprint/ and must not be clobbered.
...
latexmk -c
echo "assembled -> BBCN_paper3v3.pdf  (self-contained sibling of paper3)"
