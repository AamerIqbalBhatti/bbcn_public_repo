\section{The BBCN 141-node model}
\label{app:rules}
This appendix lists the complete Boolean update logic of the 141-node BBCN model, one block per pathway module. For each node, the right-hand side is its update rule over intra-pathway nodes and external bus inputs; \texttt{AND}, \texttt{OR}, \texttt{NOT} are the Boolean connectives, and a node whose rule equals a single input is a pass-through of that input. Two held, per-patient inputs enter the rules: \texttt{U}, the uncoupled-state flag, set to $1$ when the patient's AKT activity and nuclear FOXO3a co-occur; and \texttt{GENOTOXIC}, the DNA-damage treatment input, off by default.
...
\begin{flushleft}\ttfamily\small
MDM2 = (TP53 OR AKT1) AND NOT (CDKN2A AND (E2F1 OR ATM OR ATR OR GENOTOXIC))\\
TP53 = (NOT MDM2) OR ATM OR ATR OR GENOTOXIC\\
JUN = (MAPK8 OR MAPK1 OR RELA) AND NOT (PPARG OR DUSP1)\\
XIAP = (AKT1 OR JUN) AND NOT SMAC\\
CFLAR = (AKT1 OR RELA OR CREB1) AND TP53\\
\end{flushleft}

\paragraph{APOPTOSIS REGULATORY. (Apoptosis\_ON)}\mbox{}\\
\begin{flushleft}\ttfamily\small
MCL1 = (NOT GSK3B AND MAPK1) AND NOT NOXA\\
PAK1 = TCF7L2\\
PRKACA = AKT1 OR WNT1\\
SRC = PRKACA OR ABL1\\
\end{flushleft}
...
FADD = FAS OR TRADD\\
TRADD = FAS\\
CASP8 = (FADD AND NOT CFLAR) OR CASP6\\
BID = CASP8\\
...
BCL2 = (NOT (BAD OR BCL2L11) AND (CREB1 OR MAPK1)) AND NOT PUMA\\
BCL2L1 = (NOT (BAD OR BCL2L11) AND CREB1) AND NOT PUMA\\
...
\noindent\textit{Total: 141 nodes across 24 modules, of which 22 are regulatory pathways and two are boundary blocks carrying external inputs. The pathway graph of Section~\ref{sec:obstruction} is built on the 22 regulatory pathways.}